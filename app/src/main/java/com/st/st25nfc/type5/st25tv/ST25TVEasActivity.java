...
public class ST25TVEasActivity extends STFragmentActivity implements STType5PwdDialogFragment.STType5PwdDialogListener, NavigationView.OnNavigationItemSelectedListener {
    ...
    static public final String ST25TV_CHECK_EAS_ALARM_KEY = "st25tv_check_eas_alarm_key";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        boolean checkEasAlarm = mSharedPreferences.getBoolean(ST25TV_CHECK_EAS_ALARM_KEY, true);

        mCheckEasStatusCheckBox = findViewById(R.id.checkEasStatusCheckBox);
        mCheckEasStatusCheckBox.setChecked(checkEasAlarm);
        mCheckEasStatusCheckBox.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {

            @Override
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                mSharedPreferences.edit().putBoolean(ST25TV_CHECK_EAS_ALARM_KEY, isChecked).apply();
            }
        });
    }
    ...
    private void configureEas() {
        new Thread(new Runnable() {
            public void run() {
                ...
                try {
                    int easIdValue = Helper.convertHexStringToInt(mEasIdEditText.getText().toString());
