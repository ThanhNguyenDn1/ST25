...
public class ST25TVChangePermissionsActivity extends STFragmentActivity implements STType5PwdDialogFragment.STType5PwdDialogListener, NavigationView.OnNavigationItemSelectedListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        configurationPasswordNumber = UIHelper.getConfigurationPasswordNumber(mNFCTag);

        mMultiAreaInterface = (MultiAreaInterface) mNFCTag;
        ...
        new Thread(new Runnable() {
            public void run() {
                try {
                    ...
                    mArea1ReadWriteProtection = mMultiAreaInterface.getReadWriteProtection(1);

                    if (mMultiAreaInterface.getNumberOfAreas() == 2) {
                        mArea2ReadWriteProtection = mMultiAreaInterface.getReadWriteProtection(2);
                    } else {
                        // No AREA 2
    ...
    private void changePermission() {
        new Thread(new Runnable() {
            public void run() {
                ...
                try {
                    if (mArea1RadioButton.isChecked()) {
                        mMultiAreaInterface.setReadWriteProtection(1, readWriteProtection);
                    } else {
                        mMultiAreaInterface.setReadWriteProtection(2, readWriteProtection);
                    }

                    if (mNFCTag instanceof ST25TVCTag) {
                        DisplayTapTagRequest.run(ST25TVChangePermissionsActivity.this, mNFCTag, getString(R.string.please_tap_the_tag_again));
                    } else {
